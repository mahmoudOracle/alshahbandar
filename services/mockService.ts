...
let expenseCategories: StoredExpenseCategory[] = [];
...
export const seedData = async (companyId: string) => {
    ...
    companyUsers = [
        { uid: 'dev-owner-id', name: 'Dev Owner', email: 'owner@test.com', role: UserRole.Owner, status: 'active', createdAt: new Date(), updatedAt: new Date() },
        { uid: 'dev-manager-id', name: 'Dev Manager', email: 'manager@test.com', role: UserRole.Manager, status: 'active', createdAt: new Date(), updatedAt: new Date() },
    ];
...
export const resolveFirstLogin = async (user: User): Promise<void> => {
    await delay(100);
    // No-op for mock, as getCompanyMemberships handles it.
    return;
};
...
export const inviteUser = async (companyId: string, email: string, role: UserRole, invitedBy: { uid: string; email: string; }) => {
    await delay(150);
    const inv: CompanyInvitation = { id: `inv_${Date.now()}`, email, role, invitedByUid: invitedBy.uid, invitedByEmail: invitedBy.email, createdAt: new Date(), used: false };
