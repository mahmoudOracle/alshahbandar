...
export const getCompanyMembershipsForUid = async (
  uid: string
): Promise<{ companyId: string; membershipData: Record<string, unknown> }[]> => {
  ...
  for (const userDoc of querySnapshot.docs) {
    ...
    if (companyId)
      results.push({ companyId, membershipData: userDoc.data() as Record<string, unknown> });
  }
  return results;
};
