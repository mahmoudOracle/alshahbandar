          ...
          <>
                  ...
                  {filteredCompanies.map((company) => (
                    <tr key={company.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {(() => {
                          const status = (company as unknown as Record<string, unknown>)[
                            'status'
                          ] as string | undefined;
                          return (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {company.createdAt?.toDate
                          ? company.createdAt.toDate().toLocaleDateString('ar-EG')
                          : 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {((company as unknown as Record<string, unknown>)['status'] as
                              | string
                              | undefined) === 'approved'
                              ? 'رفض'
                              : 'موافقة'}
              ...
              {filteredCompanies.map((company) => (
                <Card key={company.id} padding="sm">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {(() => {
                      const status = (company as unknown as Record<string, unknown>)['status'] as
                        | string
                        | undefined;
                      return (
                        <Badge variant={status === 'approved' ? 'success' : 'default'}>
            ...
            <div className="flex justify-center items-center mt-6 gap-2">
              ...
              <Button
                ...
                disabled={!companiesData.nextCursor}
                variant="secondary"
                size="sm"
