...
import { PlusIcon, BuildingOffice2Icon, ChevronLeftIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
...
const PlatformCompaniesPage: React.FC = () => {
    ...
    const handleStatusToggle = async (company: Company) => {
        try {
            await updateCompanyStatus(company.id, !company.isActive);
            addNotification('تم تحديث حالة الشركة بنجاح.', 'success');
            fetchCompanies(currentPage); // Refresh data
        } catch (error) {
    ...
    const summaryStats = useMemo(() => {
        ...
        const active = companiesData.data.filter(c => c.isActive).length;
        return {
                                ...
                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    {filteredCompanies.map(company => (
                                        <tr key={company.id}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">{company.ownerEmail}</td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Button variant="secondary" size="sm" onClick={() => handleStatusToggle(company)}>
                            ...
                            {filteredCompanies.map(company => (
                                <Card key={company.id} padding="sm">
                                    ...
                                    <div className="flex gap-2 mt-3 border-t border-gray-200 dark:border-gray-700 pt-3">
                                        ...
                                        <Button variant="ghost" size="sm" className="flex-1" onClick={() => handleStatusToggle(company)}>
                                            {company.isActive ? 'إيقاف' : 'تفعيل'}
                                        </Button>
                                    </div>
                                </Card>
