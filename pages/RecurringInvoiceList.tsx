    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
            ...
            {sortedInvoices.map((rec) => (
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {new Date(rec.nextDueDate).toLocaleDateString('ar-EG')}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {rec.endDate ? new Date(rec.endDate).toLocaleDateString('ar-EG') : 'لا يوجد'}
                </td>
                {canWrite && (
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-2 rtl:space-x-reverse">
                      <Link
                        to={`/recurring/edit/${rec.id}`}
      ...
      <div className="md:hidden space-y-4">
        {sortedInvoices.map((rec) => (
          <div
            key={rec.id}
            className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg shadow hover:shadow-md transition-shadow"
          >
