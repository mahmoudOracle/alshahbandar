  ...
  return (
    <Card>
      ...
      <div className="mb-4">
        <div className="flex gap-2">
          <select
            aria-label="product-add-select"
            className="p-2 border rounded flex-1"
            onChange={(e) => {
              const prod = products.find((p) => p.id === e.target.value);
              if (e.target.value) addRow(prod);
            }}
          >
      ...
      {items.length === 0 && (
        <EmptyState
          title="لا يوجد عناصر"
          message="أضف منتجات إلى أمر الشراء."
          action={{ text: 'أضف عنصر', onClick: () => addRow() }}
        />
      )}
              ...
              {items.map((it, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  <td className="px-4 py-2">
                    <select
                      aria-label="purchase-row-product-select"
                  ...
                  <td className="px-4 py-2">
                    <Input
                      aria-label="purchase-row-quantity"
                      type="number"
                      min={1}
                      value={it.quantity}
                  ...
                  <td className="px-4 py-2">
                    <Input
                      aria-label="purchase-row-unitPrice"
                      type="number"
                      min={0}
                      value={it.unitPrice}
