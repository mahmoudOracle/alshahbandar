...
function ExpenseChartLoader({
  ...
  useEffect(() => {
    ...
    import('recharts')
      .then((mod) => {
        if (mounted) setR(mod as unknown);
      })
      .catch(() => {});
    return () => {
      mounted = false;
    ...
    <Card header={<h2 className="text-xl font-bold">ملخص المصروفات (هذا الشهر)</h2>}>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1 flex flex-col items-center justify-center bg-danger-50 dark:bg-danger-900/30 p-4 rounded-lg">
          ...
          <p className="text-3xl font-bold mt-2 text-danger-600">
            {totalThisMonth.toFixed(2)} {settings?.currency}
          </p>
        </div>
...
}> = ({ expense, currency, canWrite, onDelete }) => (
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {filteredExpenses.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-semibold text-danger-600">
