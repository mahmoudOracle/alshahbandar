    ...
    <form onSubmit={handleSubmit}>
      <Card>
        <fieldset disabled={!canWrite} className="space-y-6">
          ...
          <div className="border-t pt-6">
            <h3 className="text-lg font-semibold mb-4">الضرائب</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
              <div>
                <Input label="نسبة الضريبة (%)" type="number" value={invoice.taxRate ?? 0} onChange={e => dispatch({ type: 'UPDATE_FIELD', payload: { field: 'taxRate', value: parseFloat(e.target.value) } })} />
              </div>
              <div className="text-right">
                <p className="text-sm text-gray-600">قيمة الضريبة: <span className="font-medium">{taxAmount.toFixed(2)} {settings?.currency}</span></p>
              </div>
            </div>
          </div>
        </fieldset>
