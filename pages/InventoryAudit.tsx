                ...
                {inventory.map((i) => (
                  <tr key={i.productId} className="border-t">
                    ...
                    <td className="p-2 text-right">
                      {i.updatedAt ? new Date(i.updatedAt as any).toLocaleString() : '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                onClick={() => {
                  ...
                  const rows = [
                    ['productId', 'quantity', 'type', 'unitCost', 'reference', 'createdAt'],
                  ];
                  ...
                  for (const l of filtered)
                    rows.push([
                      ...
                      String(l.type),
                      String(
                        (l.unitCost || 0).toFixed
                          ? (l.unitCost as any).toFixed(2)
                          : String(l.unitCost || 0)
                      ),
                      String(l.referenceId || ''),
                      l.createdAt ? new Date(l.createdAt as any).toISOString() : '',
                    ]);
                  const csv = rows
                  ...
                  a.href = url;
                  a.download = `stock-ledger-${Date.now()}.csv`;
                  a.click();
            ...
            <div className="overflow-auto max-h-96">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {(() => {
                    const filtered = ledger.filter((l) => {
                      if (productFilter && !String(l.productId).includes(productFilter))
                        return false;
                      if (typeFilter && String(l.type) !== typeFilter) return false;
                      return true;
                    });
                    ...
                    return paged.map((l) => (
                      <tr key={l.id} className="border-t">
                        ...
                        <td className="p-2 text-right">
                          {l.referenceCollection || ''} / {l.referenceId || ''}
                        </td>
                        <td className="p-2 text-right">
                          {l.createdAt ? new Date(l.createdAt as any).toLocaleString() : '-'}
                        </td>
                      </tr>
                    ));
                  })()}
