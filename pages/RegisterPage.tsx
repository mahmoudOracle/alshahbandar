...
const RegisterPage: React.FC = () => {
  ...
  const handleDetailsSubmit = (e: React.FormEvent) => {
    ...
    if (
      ...
      addNotification(
        'برجاء ملء جميع الحقول المطلوبة (بما في ذلك الدولة، المدينة، ورقم الهاتف).',
        'error'
      );
      return;
    }
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const getIdToken = (user as { getIdToken?: (force?: boolean) => Promise<string> })
          ?.getIdToken;
      ...
      try {
        ...
        try {
          ...
          console.warn(
            '[Register] createOwnerCompanyCallable failed, falling back to client write',
            callErr
          );

          // Ensure token propagation: refresh token and wait briefly
          try {
            const refresh = (user as { getIdToken?: (force?: boolean) => Promise<string> })
              ?.getIdToken;
          ...
          try {
            ...
            console.error(
              '🔴 [Register] Failed to create company document for uid (client write):',
              user.uid,
              createErr
            );
      ...
      if (error instanceof FirebaseError) {
        console.error(
          '[DEBUG][Register] FirebaseError code:',
          error.code,
          'message:',
          error.message
        );
        switch (error.code) {
          case 'auth/configuration-not-found':
            message =
              'إعدادات مصادقة Firebase غير مكتملة. يرجى تمكين طريقة تسجيل البريد الإلكتروني/كلمة المرور في وحدة تحكم Firebase.';
            break;
