...
const getStatusChip = (status: QuoteStatus) => {
  switch (status) {
    case QuoteStatus.Draft:
      return (
        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 dark:text-gray-200 dark:bg-gray-600">
          مسودة
        </span>
      );
    case QuoteStatus.Sent:
      return (
        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800 dark:text-blue-200 dark:bg-blue-700">
          مرسل
        </span>
      );
    case QuoteStatus.Accepted:
      return (
        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 dark:text-green-200 dark:bg-green-700">
          مقبول
        </span>
      );
    case QuoteStatus.Declined:
      return (
        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800 dark:text-red-200 dark:bg-red-700">
          مرفوض
        </span>
      );
  }
        ...
        <>
                ...
                {filteredQuotes.map((quote) => (
                  <tr key={quote.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {new Date(quote.date).toLocaleDateString('ar-EG')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {new Date(quote.expiryDate).toLocaleDateString('ar-EG')}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {quote.total.toFixed(2)} {settings?.currency}
                    </td>
          ...
          <div className="md:hidden space-y-4">
            {filteredQuotes.map((quote) => (
              <div
                key={quote.id}
                className="bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg shadow-md hover:shadow-lg"
              >
