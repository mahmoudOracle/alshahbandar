...
describe('createInvoiceAtomic (integration)', () => {
  if (!emulatorAvailable) {
    it('skipped - Firebase emulator not configured', () => {
      console.warn(
        'Skipping createInvoiceAtomic integration tests: set FIRESTORE_EMULATOR_HOST and start emulators to run them.'
      );
  ...
  it('creates invoice atomically and updates stock & ledger (emulator required)', async () => {
    ...
    const invoiceId =
      json && json.result && json.result.invoiceId
        ? json.result.invoiceId
        : (json && json.invoiceId) || (json && json.data && json.data.invoiceId);
    ...
    const ledgerSnap = await companyRef
      .collection('stockLedger')
      .where('productId', '==', productId)
      .where('referenceId', '==', String(invoiceId))
      .get();
