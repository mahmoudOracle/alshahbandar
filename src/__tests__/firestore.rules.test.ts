...
try {
  // Dynamically require to avoid throwing when dev deps are not installed
  const ru = require('@firebase/rules-unit-testing');
...
describe('Firestore security rules - stockLedger & inventory', () => {
  beforeAll(async () => {
    ...
    testEnv = await initializeTestEnvironment({ projectId: PROJECT_ID, firebaseConfig: { projectId: PROJECT_ID }, rules: fs.readFileSync('firestore.rules', 'utf8') });
  });
  ...
  it('prevents non-admin clients from creating stockLedger entries', async () => {
    ...
    const write = aliceDb.collection('companies').doc('comp1').collection('stockLedger').doc('entry1').set({
      productId: 'p1',
      change: -2,
      qtyBefore: 5,
      qtyAfter: 3,
  ...
  it('allows admin-claim clients to create valid stockLedger entries', async () => {
    if (!testsAvailable) return;
    const admin = testEnv.authenticatedContext('admin', { uid: 'admin', email: 'admin@test.com', token: { admin: true } });
    const adminDb = admin.firestore();

    const write = adminDb.collection('companies').doc('comp1').collection('stockLedger').doc('entry2').set({
      productId: 'p1',
      change: 10,
      qtyBefore: 0,
      qtyAfter: 10,
  ...
  it('prevents invalid qty arithmetic in ledger entries', async () => {
    if (!testsAvailable) return;
    const admin = testEnv.authenticatedContext('admin2', { uid: 'admin2', email: 'a2@test.com', token: { admin: true } });
    const adminDb = admin.firestore();

    const bad = adminDb.collection('companies').doc('comp1').collection('stockLedger').doc('entry3').set({
      productId: 'p1',
      change: -5,
      qtyBefore: 2,
