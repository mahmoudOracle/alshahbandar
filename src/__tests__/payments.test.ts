...
describe('payments and invoice balance (mock)', () => {
  ...
  it('updates invoice paymentsSummary on partial and full payments', async () => {
    const prod = await mockService.saveProduct('mock-company', { name: 'Pack', description: '', price: 20, stock: 10, defaultCost: 10 });
    const inv = {
      customerId: 'custX', customerName: 'C X', date: new Date().toISOString().split('T')[0], dueDate: new Date().toISOString().split('T')[0],
      items: [{ id: 'li', productId: prod.id, productName: prod.name, quantity: 2, price: 20 }], subtotal: 40, total: 40, paymentType: 'كاش' as any, status: 'Due' as any
    } as any;

    const savedInv = await saveInvoice('mock-company', inv);
    expect(savedInv.paymentsSummary?.paid).toBe(0);

    const p1 = await savePayment('mock-company', { invoiceId: savedInv.id, customerId: savedInv.customerId, amount: 15, date: new Date().toISOString() });
    ...
    const p2 = await savePayment('mock-company', { invoiceId: savedInv.id, customerId: savedInv.customerId, amount: 25, date: new Date().toISOString() });
