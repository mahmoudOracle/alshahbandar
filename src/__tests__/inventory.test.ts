...
describe('inventory and stock ledger (mock)', () => {
  ...
  it('saveGoodsReceipt increases product stock, creates inventory snapshot and ledger entries', async () => {
    const prod = await mockService.saveProduct('mock-company', { name: 'Lipstick A', description: '', price: 15, stock: 0 });
    const res = await saveGoodsReceipt('mock-company', { supplierId: 's1', items: [{ productId: prod.id, quantity: 20 }] });
  ...
  it('createPurchase updates stock and ledger with unitCost', async () => {
    const prod = await mockService.saveProduct('mock-company', { name: 'Cream B', description: '', price: 25, stock: 5 });
    const pRes = await createPurchase('mock-company', { supplierId: 's2', items: [{ productId: prod.id, quantity: 10, unitPrice: 9.5 }], totalAmount: 95 });
